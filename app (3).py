    ...
    def download_first_visible(self, label_for_log: str) -> Optional[Path]:
        ...
        if not controls:
            ...
            if coords:
                ...
                return None
            raise RuntimeError(f"No download control found for '{label_for_log}'.")
...
def render_step_1() -> None:
    ...
    folder_val = st.text_input("📂 Open a specific folder first? (optional, type its on-screen label)",
                                value=S.folder_label, placeholder="leave blank to search the whole portal")
    ...
    with bc1:
        if st.button("Next: Open Login Page →", use_container_width=True, disabled=not SELENIUM_AVAILABLE):
            ...
            if not url_val.strip():
                ...
                S.folder_label = folder_val.strip()
...
def render_step_3() -> None:
    ...
    st.caption(f"Terms: {', '.join(S.search_terms)}")
    st.markdown("</div>", unsafe_allow_html=True)
